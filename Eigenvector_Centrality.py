...
def eigenvector_centrality_analysis(grafo, num_nodi=5000):
    """
    Analisi della Eigenvector Centrality per i primi num_nodi del grafo.
    """
    print(f"Analisi Eigenvector Centrality per i primi {num_nodi} nodi...")
    sub_grafo = grafo.subgraph(list(grafo.nodes)[:num_nodi])
    centrality = nx.eigenvector_centrality(sub_grafo, max_iter=500)
    ...
    nx.draw(sub_grafo, pos, node_size=np.array([v * 1000 for v in centrality.values()]),
            node_color='purple', with_labels=False)
    plt.title("Grafo basato sulla Eigenvector Centrality")
    plt.show()

    plt.figure(figsize=(10, 6))
    plt.bar(range(len(centrality)), list(centrality.values()), color='purple')
    plt.title("Istogramma della Eigenvector Centrality")
    plt.xlabel("Nodi")
...
if __name__ == "__main__":
    ...
    eigenvector_centrality_analysis(grafo)
