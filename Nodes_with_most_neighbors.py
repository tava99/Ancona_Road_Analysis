...
if __name__ == "__main__":
    ...
    # Funzione per trovare e visualizzare i nodi con più vicini
    def nodes_with_most_neighbors(grafo, top_n=5, num_nodi=5000):
        """
        Trova i nodi con il maggior numero di vicini.
        """
        sub_grafo = grafo.subgraph(list(grafo.nodes)[:num_nodi])

        print(f"Calcolo dei {top_n} nodi con il maggior numero di vicini...")
        degrees = sub_grafo.degree()
        top_nodes = sorted(degrees, key=lambda x: x[1], reverse=True)[:top_n]
        print(f"Nodi con più vicini: {top_nodes}")

        # Visualizza i nodi con più vicini
        pos = nx.spring_layout(sub_grafo)
        plt.figure(figsize=(12, 12))
        nx.draw(sub_grafo, pos, with_labels=True, node_color='lightgray', edge_color='lightgray')
        nx.draw_networkx_nodes(sub_grafo, pos, nodelist=[n[0] for n in top_nodes], node_color='blue', node_size=500)
        plt.title(f"Top {top_n} nodi con più vicini")
        plt.show()
    ...
    # Analisi dei nodi con più vicini
    nodes_with_most_neighbors(grafo, top_n=5, num_nodi=5000)
